...
==== Example of incorrect param value
...
|billboards[].centerFocusImage |String |true |Center-focused crop image URL (optional).
|billboards[].url |String |false |Billboard link URL. May be empty.
|billboards[].premium |Boolean |false |Whether this billboard is premium-only (national billboards).
|===
...
|items[].description |String |true |Plain text description (optional).
|items[].equipment |String |true |Required equipment.
...
|items[].circle_image |String |false |Circle image URL.
|items[].instructor |String |true |Instructor name.
|items[].instructorId |String |true |Instructor ID.
|items[].instructors[] |Array[Object] |false |Instructor objects.
|items[].instructors[].name |String |true |Instructor name.
...

{
    "items": [ {
        ...
        "duration": 3600,
        "thumbnail": "https://cms.ymca360.org/sites/default/files/live-stream/1/thumb-small.jpg",
        "circle_image": "https://cms.ymca360.org/sites/default/files/live-stream/1/thumb-small.jpg",
        "premium": false,
        "instructor": "Corri Lewellen",
        "instructorId": "12",
        "instructors": [
...
|liveStreamDetails.chatEnabled |Boolean |false |Whether live chat is enabled for this stream.
|liveStreamDetails.source |Array[Object] |false |Playback sources (empty when status is `offline`).
|liveStreamDetails.source[].type |String |false |Stream type (currently always `m3u8`).
|liveStreamDetails.source[].id |String |false |Internal stream tracker ID.
|liveStreamDetails.source[].url |String |false |Playback URL. For Vimeo streams this is the vimeo-playback proxy URL.
|liveStreamDetails.source[].provider |String |false |Stream provider. `vimeo` for Vimeo-hosted streams; absent for IVS streams.
|liveStreamDetails.source[].vimeoEventId |Integer |false |Vimeo event ID. Only present when `provider` is `vimeo`.
...
|liveStreamBrief.circle_image |String |false |Circle image URL.
...
|liveStreamBrief.description |String |true |Plain text description (optional).
|liveStreamBrief.equipment |String |true |Required equipment.
...

{
    "liveStreamBrief": {
        ...
        "program": 22,
        "programName": "Kids' Acitivities",
        "description": "A live yoga session for kids.",
        "equipment": "",
        "association": "Greater Wichita YMCA",
        "language": "English",
        "restream": true,
    ...
    "liveStreamDetails": {
        ...
        "source": [ {
            "type": "m3u8",
            "id": "VyPAdfNsZYHj",
            "url": "https://a3a9497e12e9.us-east-1.playback.live-video.net/api/video/v1/us-east-1.806120511164.channel.VyPAdfNsZYHj.m3u8"
        }]
    }
}
----
...
HTTP/1.1 200 OK
{
  "liveStreamBrief": { "...": "same fields as above" },
  "liveStreamDetails": {
    "status": "live",
    "descriptionHtml": "",
    "equipment": "",
...
Returns a 302 redirect to a fresh signed m3u8 playback URL for a Vimeo-hosted live stream. The signed URL is short-lived; clients must follow this proxy endpoint each time rather than caching the redirect target.
...
|ls_association_machine_name |String |Yes |Association machine name
|livestream_id |Integer |Yes |Live stream node ID
|===
...
|302 |Redirect (in `Location` header) to a time-limited signed m3u8 playback URL
|401 |Missing or invalid API key
|404 |Live stream not found, or not a Vimeo-hosted live stream
|===
...
| association.billboards[].url |String |false |Billboard link URL. May be empty.
| association.billboards[].centerFocusImage |String |false |Center-focused crop image URL (optional).
| association.billboards[].premium |Boolean |false |Whether this billboard is premium-only (national billboards).
...
[source,json]
----
$ curl 'http://localhost:8080/api/virtual-classes/v4.0/content-providers/wichita/combined/frontpage' -i -X GET \
    -H 'Accept: application/json' -H 'authorization: apiKey 0a47c3bf-4740-465d-a22e-0b25ef86ddd0'
----
...
[source,json]
----
$ curl 'http://localhost:8080/api/virtual-classes/v4.0/content-providers/wichita/combined/watchscreen' -i -X GET \
-H 'Accept: application/json' -H 'authorization: apiKey 0a47c3bf-4740-465d-a22e-0b25ef86ddd0'
----
...
WARNING: The Widgets API endpoints below are documented but *not yet implemented* in the current CMS codebase.
